...
# ═══════════════════════════════════════════════════════════════════
def generate_snowflake_queries_safe(schema: str, task: str) -> dict:
    """
    Sends a natural language task to Llama and returns a structured JSON result.

    Args:
        schema (str): The database schema description.
        task   (str): A natural language task to convert to snowflake queries.

    Returns:
        dict: {
            "task"       : original task,
            "snowflake_query"  : generated snowflake query,
...
{{
  "snowflake_query"  : "<the snowflake query as a single string>",
  "explanation": "<plain English explanation of what the snowflake query does>"
}}
"""
    try:
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Generate clean, standard snowflake queries for the given task. "
        ...
        return {
            "task"       : task,
            "snowflake_query"  : parsed.get("snowflake_query", ""),
...
if __name__ == "__main__":
    ...
    for i, task in enumerate(tasks, start=1):
        ...
        result = generate_snowflake_queries_safe(schema, task)
